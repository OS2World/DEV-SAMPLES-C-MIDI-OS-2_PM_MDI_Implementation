...
// ------------------------------------------
   USHORT far _loadds TellUser (idMsg, pszDLLName, flStyle, ...)
   ...
   // Define function data
     ...
     {CHAR szFormat[MAX_STRING];       // Message format
      ...
      HMODULE hmod = (HMODULE) NULL;   // Requesting module handle
   ...
   // Extract the message caption from the DLL name.
      ...
      if ((pszDot = strrchr (szCaption, '.')) != (PVOID) NULL)
        *pszDot = 0;
   ...
   // Load the message string
      ...
      WinLoadString ((HAB) NULL, hmod, idMsg, sizeof (szFormat), szFormat);
