...
// -----------------------------------------
   ...
   // Attempt to allocate new memory; issue an error message on failure
      ...
      while ((pvData = malloc ((size_t) cbSize)) == (PVOID) NULL)
        {if (WinMessageBox (HWND_DESKTOP, HWND_DESKTOP, pszMemErr, (PSZ) NULL, 0,
...
// --------------------------------------------------
   ...
   // Query memory size
     ...
     {return (pvData == (PVOID) NULL)? 0L : (ULONG) _msize (pvData);
...
// ---------------------------------------------
   ...
   // Attempt to allocate new memory; issue an error message on failure
      ...
      while ((pvNew = realloc (pvOld, (size_t) cbSize)) == (PVOID) NULL)
        {if (WinMessageBox (HWND_DESKTOP, HWND_DESKTOP, pszMemErr, (PSZ) NULL, 0,
          MB_ABORTRETRYIGNORE | MB_ICONHAND) == MBID_ABORT)
           DosExit (EXIT_PROCESS, 0);
        }
